...
def context_columns(df, pollutant):
    ...
    # Meteorological covariates (ERA5-derived), per manuscript Section 3.2:
    # "temperature, relative humidity, surface pressure, precipitation, wind
    # speed, and boundary-layer height... used as the contextual covariates
    # against which each pollutant's expected value is estimated." These are
    # only present after 04_match_integrate_era5.py has been run; included
    # here when available rather than assumed, so this function still works
    # (producing a documented, weaker context set) on the non-ERA5 base master.
    met_cols = ["temperature_c", "relative_humidity_pct", "surface_pressure_hpa",
                "precipitation_mm", "wind_speed_ms", "boundary_layer_height_m"]
